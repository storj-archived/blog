...
This post discusses the very important qualification gates we’ve established for each of our major milestones: Pioneer 1 (beta 1), Pioneer 2 (beta 2), and Voyager (production). It also discusses where we stand in terms of meeting those gates.
...
We measure the time it takes to upload a 10 MB file and then we repeat this test for 500 files. We then compare the results to AWS, which is generally considered the gold standard for centralized cloud service. We not only look at the median time to upload, we also look at the long tail performance. Not only are we excited that we’re more than twice the speed of AWS S3, but we also have a much tighter distribution. Our 95th percentile time is only a quarter of a second slower than our median. This points to the power of decentralization and should only get better as we add more nodes and continue to add nodes that are distributed closer to the end users (ultimately, the speed of light becomes a factor).
...
We measure the time it takes to download and reconstitute a 10 MB file. We repeat this test for 500 files, and then compare the results to AWS. Our median download time is 2.18 seconds. We’re especially excited about our tight distribution. This points to the power of decentralization, where the network isn’t vulnerable to the impact of random, slow nodes. And, both speed and variability should only get better as we add more nodes and add nodes that are distributed closer to the end users (ultimately, the speed of light becomes a factor).
...
Both Storj lab and our partners have uploaded over 2 PB of data to the network. Our production goal is to be at 6 PB, and we expect to exceed that goal by a wide margin.

  

Note that while we believe the capacity over the network is significantly higher, we held ourselves to this more conservative, proven number. This number is more than an order of magnitude lower than the V2 network, which had a capacity of 150 PB at its peak. While we have several partners and beta customers with several petabytes of capacity, we’re aiming to grow the network more gradually, so that we generally only have about three months of excess capacity at a time. This helps us ensure that all nodes are receiving economically interesting payouts. 
...
Our current node churn (excluding probationary nodes) is about 2.4%, implying a mean node lifetime of 7.4 months. Our beta 1 gate was 5%. We’re not yet at our production goal of 2% (implying a 11.1 month average node lifetime). Our system is very resilient to having any individual nodes or even a significant percentage of nodes churn. However, performance, economics, and most statistics all do better as we bring average node churn down.
