...
Although we get a huge lift on performance from the bittorrent-style parallelism, (especially with downloads, the speeds can be blazing fast), uploads have the additional challenge of the erasure coding overhead. When a 1 GB file is uploaded, we’re not just uploading the 1 GB, we’re also [uploading another 1.75 GB of additional data from the erasure coding](https://storj.io/blog/2019/01/why-proof-of--replication-is-bad-for-decentralized-storage-part-2-churn-and-burn/) to provide the required level of durability. What that means is we need a lot of storage node operators with fast download speeds so we can provide high performance uploads even with the extra data overhead. 
